🖐️ Hi! I'm Anton. 
# About me:
* Electrical Engineer
* A python 🐍 programmer with 8+ years of experience and a passion for mobile and desktop application.
* C++/C rising developer (3+ Years of experience, still learning)
* Proficient Linux/Linux Servers. 
* AI power user
* Support open source. Happy to help with any project to the best of my ability. 
# My current Projects: 

1. 🖥️ [GamerNexus](https://github.com/MrAntonS/For-Gamers)

# My other works:

1. 🤖 [Deep Learning from scratch](https://github.com/MrAntonS/ML22) | Numpy | Python
2. ✈️ Airplane Tracker(Old) | Python
3. [ConvNet](https://github.com/MrAntonS/Convenient-Networking), a terminal emulator
4. 📱 [Couples App(Client, and Server) | C++](https://github.com/smower476/couples_app)
5. 📱 [Thunder | Dart](https://github.com/hjiangsu/thunder)

### I am always looking for new opportinities.
LinkedIn: [Anton Saenko](https://www.linkedin.com/in/anton-saenko/)\
Mastodon: [BigBon](https://techhub.social/@bigbon)\
Telegram: [@antonSaenk0](https://t.me/antonSaenk0) 
